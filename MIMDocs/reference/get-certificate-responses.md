---
title: Obter respostas de certificado | Microsoft Docs
description: ''
keywords: ''
author: billmath
ms.author: billmath
manager: daveba
ms.date: 09/26/2017
ms.topic: reference
ms.prod: microsoft-identity-manager
ms.assetid: b0559e7b-eaff-499b-8bcf-c6cdf5a89545
audience: developer
ms.reviewer: mwahl
ms.suite: ems
ms.openlocfilehash: 75c92bd31cda28c5eaef7d16832c819e27737525
ms.sourcegitcommit: 7e8c3b85dd3c3965de9cb407daf74521e4cc5515
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 03/10/2020
ms.locfileid: "92757442"
---
# <a name="get-certificate-responses"></a>Obter respostas de certificado
Envia um conjunto de solicitações de certificado para a CA (Autoridade de Certificação) para processamento.

>[!IMPORTANT]
>O número de respostas de certificado pode exceder o número de solicitações. Alguns modelos de perfil podem ser configurados para gerar chaves/solicitações no servidor ou ter certificados externos.

>[!NOTE]
>As URLs neste artigo são relativas ao nome de host escolhido durante a implantação da API, como `https://api.contoso.com` .

## <a name="request"></a>Solicitação

Método  |URL da solicitação  
---------|---------
POST     |/CertificateManagement/api/v1.0/requests/{id}/certificatedata

### <a name="url-parameters"></a>Parâmetros de URL

Parâmetro | Descrição
--------|--------------
id| Necessário. O GUID correspondente ao modelo de perfil do qual extrair a política.

### <a name="request-headers"></a>Cabeçalhos de solicitação
Para cabeçalhos de solicitação comuns, consulte [cabeçalhos de solicitação e resposta http](certificate-management-rest-api-service-details.md#http-request-and-response-headers) nos *detalhes do serviço da API REST cm* .

### <a name="request-body"></a>Corpo da solicitação
O corpo da solicitação contém as seguintes propriedades:

Propriedade | Descrição
---------|-----------
pfxpassword | A senha que é usada para proteger quaisquer resultados de troca de informações pessoais (PFX) retornados da chamada.
certificaterequests | A lista de solicitações de certificado que são geradas pelo cliente. As solicitações devem ser geradas usando os parâmetros especificados no objeto CertificateRequestGenerationOptions. Para obter mais informações, consulte [Opções obter gerações de solicitação de certificado](get-certificate-request-generation-options.md).
comments | Uma cadeia de caracteres que contém quaisquer comentários adicionais que são registrados no histórico de eventos da operação.


## <a name="response"></a>Resposta
Esta seção descreve a resposta.

### <a name="response-codes"></a>Códigos de resposta

Código  |Descrição  
---------|---------
200 | OK
403 | Proibido
500 | Erro Interno

### <a name="response-headers"></a>Cabeçalhos de resposta
Para cabeçalhos de resposta comuns, consulte [cabeçalhos de solicitação e resposta http](certificate-management-rest-api-service-details.md#http-request-and-response-headers) nos *detalhes do serviço da API REST cm* .

### <a name="response-body"></a>Corpo da resposta
Em caso de sucesso, retorna uma lista de objetos [Microsoft. CLM. Shared. Certificates. Certificate](https://msdn.microsoft.com/library/microsoft.clm.shared.certificates.certificate.aspx) serializados em JSON com as seguintes propriedades:

Propriedade| Descrição
------|-------------
CertificateId | A ID do certificado.
Erro | Qualquer erro encontrado ao processar a solicitação de certificado.
Flags | Os sinalizadores que descrevem o certificado.
IsExternal | Um valor booliano que indica se o certificado é um certificado externo.
isKeyHistory | Um valor booliano que indica se o certificado é um certificado histórico de chave.
isPfx | Um valor booliano que indica se a resposta da solicitação de certificado está no formato PFX.
isPkcs7 | Um valor booliano que indica se a resposta da solicitação de certificado está no formato PKCS#7.
isServerGenerated | Um valor booliano que indica se o certificado foi gerado por um servidor.
Pfx | O blob de certificado PFX.
Pkcs7 | O blob do certificado PKCS#7.
TemplateCommonName | O nome comum do modelo de certificado para o certificado.
UseLocalMachineStore | Um valor booliano que indica se o certificado é para o repositório de certificados do computador local.

## <a name="example"></a>Exemplo
Esta seção fornece um exemplo para enviar um conjunto de solicitações de certificado à autoridade de certificação para processamento.

### <a name="example-request"></a>Exemplo: solicitação

```
POST /CertificateManagement/api/v1.0/requests/C6BAD97C-F97F-4920-8947-BE980C98C6B5/certificatedata HTTP/1.1

{
    "pfxpassword":"6b75162c-b3b4-4fc1-a653-f12dae98f0c3",
    "certificaterequests":[
        "\r\n"
    ]
}
```

### <a name="example-response"></a>Exemplo: resposta

```
HTTP/1.1 200 OK

[
    {
        "TemplateCommonName":"ContosoVirtualSmartCardLimitedRelease-KSP",
        "CertificateId":0,
        "Flags":65536,
        "UseLocalMachineStore":false,
        "Error":null,
        "Pkcs7":"\r\n",
        "Pfx":null,
        "IsExternal":false,
        "isServerGenerated":false,
        "isPfx":false,
        "isPkcs7":true,
        "isKeyHistory":false
    }
]
```

## <a name="see-also"></a>Veja também

- [ MétodoMicrosoft.Clm.Provision.ExecuteOperations. Enroll](https://msdn.microsoft.com/library/microsoft.clm.provision.executeoperations.enroll.aspx)
- [Classe Microsoft. CLM. Shared. Certificates. Certificate](https://msdn.microsoft.com/library/microsoft.clm.shared.certificates.certificate.aspx)
